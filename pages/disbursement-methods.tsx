...
export default function DisbursementMethods() {
  ...
  const [filter, setFilter] = useState<'all' | 'active' | 'va' | 'disbursement'>('all');
  ...
  const toggleField = async (bank: BankCode, field: 'support_va' | 'support_disbursement' | 'is_active') => {
  ...
  const filtered = useMemo(() => {
    return banks.filter((b) => {
      ...
      if (filter === 'va' && !b.support_va) return false;
      if (filter === 'disbursement' && !b.support_disbursement) return false;
      if (search) {
  ...
  const stats = useMemo(() => {
    return {
      ...
      va: banks.filter((b) => b.support_va && b.is_active).length,
      disbursement: banks.filter((b) => b.support_disbursement && b.is_active).length,
    };
  }, [banks]);
  ...
  return (
    <Layout>
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1 text-sm">Daftar bank dengan dukungan VA &amp; disbursement.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard label="VA Support" value={stats.va} color="text-blue-600" />
        <StatCard label="Disbursement" value={stats.disbursement} color="text-indigo-600" />
      </div>
      ...
      <div className="card p-4 mb-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <select value={filter} onChange={(e) => setFilter(e.target.value as any)} className="input-field md:col-span-2">
            <option value="all">All banks</option>
            <option value="active">Active only</option>
            <option value="va">VA enabled</option>
            <option value="disbursement">Disbursement enabled</option>
          </select>
        </div>
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((b) => (
                <tr key={b.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-center">
                    <Toggle checked={b.is_active} onClick={() => toggleField(b, 'is_active')} />
                  </td>
